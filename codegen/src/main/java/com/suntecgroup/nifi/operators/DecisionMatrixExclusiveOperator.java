...
/**
 * DecisionMatrixExclusiveOperator - A class for implementing from
 * DecisionMatrixExclusiveOperator.
 * 
 */
@Component
public class DecisionMatrixExclusiveOperator implements NifiProcessorInterface {
	private static final Logger LOGGER = LoggerFactory.getLogger(DecisionMatrixExclusiveOperator.class);
	...
	@Autowired
	private NifiDecisionMatrixExclusiveProcessor nifiDecisionMatrixExclusiveProcessor;
	...
	public SuntecOperatorModel generateNifiProcessors(BPFlowUI bPFlowRequest, SuntecNiFiModel theTarget,
		...
		LOGGER.info("Create Decision Matrix Exclusive Operator Request ::");

		SuntecOperatorModel som = new SuntecOperatorModel();

		TemplateProcessor customFailureProcessor = null;
		TemplateProcessor decisionMatrixExclusiveProcessor = null;
		...
		try {
			...
			decisionMatrixExclusiveProcessor = nifiDecisionMatrixExclusiveProcessor
					.generateDecisionMatrixExclusiveProcessorReq(bPFlowRequest, theTarget, currentOperator,1);
			som.getNifiProcessorList().add(decisionMatrixExclusiveProcessor);
			...
			TemplateConnection conn_cs_to_updateTxnihttp = connectionsNifiOperator.generateNiFiConnection(
					...
					"customFailure to updateTxnInvokeHttp", theTarget);
			som.getNifiConnectionList().add(conn_cs_to_updateTxnihttp);

			TemplateConnection conn_DM_Ex_to_failure = connectionsNifiOperator.generateNiFiConnection(
					decisionMatrixExclusiveProcessor, customFailureProcessor, CGConstants.FAILURE,
					"DecisionMatrixExclusive to customFailure", theTarget);
			som.getNifiConnectionList().add(conn_DM_Ex_to_failure);

			som.setMyFirstProcessor(decisionMatrixExclusiveProcessor);
			som.setMyLastProcessor(decisionMatrixExclusiveProcessor);

		} catch (Exception e) {
			throw new CGException("Exception while Creating DecisionMatrix Exclusive Operator::", e);
		}
		LOGGER.info("DecisionMatrix Exclusive Operator Request generated");
