...
/*
 * This class is for creating a custom NiFi processor to read files from input file location for Channel Integration.
 * This class would read files and send completed files to BP flow.
 * 
...
@WritesAttributes({
		...
		@WritesAttribute(attribute = "path", description = "The path is set to the relative path of the file's directory on disk. For example, "
				+ "if the  property is set to /tmp, files picked up from /tmp will have the path attribute set to ./. If "
				+ "the  property is set to true and a file is picked up from /tmp/abc/1/2/3, then the path attribute will "
				+ "be set to abc/1/2/3"),
...
public class CustomGetFile extends AbstractProcessor {
	...
	public static final PropertyDescriptor KEEP_SOURCE_FILE = new PropertyDescriptor.Builder().name("Keep Source File")
	...
	public static final PropertyDescriptor FILE_FILTER = new PropertyDescriptor.Builder().name("File Filter")
			.description("Only files whose names match the given regular expression will be picked up").required(true)
			.defaultValue("[^\\.].*").addValidator(StandardValidators.REGULAR_EXPRESSION_VALIDATOR).build();
	public static final PropertyDescriptor PATH_FILTER = new PropertyDescriptor.Builder().name("Path Filter")
			.description("When " + RECURSE.getName()
					+ " is true, then only subdirectories whose path matches the given regular expression will be scanned")
			.required(false).addValidator(StandardValidators.REGULAR_EXPRESSION_VALIDATOR).build();
	public static final PropertyDescriptor MIN_AGE = new PropertyDescriptor.Builder().name("Minimum File Age")
			.description(
			...
			.required(true).addValidator(StandardValidators.TIME_PERIOD_VALIDATOR).defaultValue("0 sec").build();
	public static final PropertyDescriptor MAX_AGE = new PropertyDescriptor.Builder().name("Maximum File Age")
			.description(
			...
			.required(false).addValidator(StandardValidators.createTimePeriodValidator(100, TimeUnit.MILLISECONDS,
					Long.MAX_VALUE, TimeUnit.NANOSECONDS))
			.build();
	public static final PropertyDescriptor MIN_SIZE = new PropertyDescriptor.Builder().name("Minimum File Size")
			.description("The minimum size that a file must be in order to be pulled").required(true)
			.addValidator(StandardValidators.DATA_SIZE_VALIDATOR).defaultValue("0 B").build();
	public static final PropertyDescriptor MAX_SIZE = new PropertyDescriptor.Builder().name("Maximum File Size")
			.description("The maximum size that a file can be in order to be pulled").required(false)
	...
	public static final PropertyDescriptor IGNORE_HIDDEN_FILES = new PropertyDescriptor.Builder()
			.name("Ignore Hidden Files").description("Indicates whether or not hidden files should be ignored")
			.allowableValues("true", "false").defaultValue("true").required(true).build();
	 ...
	 * required(true)
	 * .addValidator(StandardValidators.POSITIVE_INTEGER_VALIDATOR).defaultValue
	 * ("10").build();
	 */
	...
	public static final Relationship REL_BACKUP = new Relationship.Builder().name("Backup")
			.description("All files are routed to Backup").build();

	private List properties;
	private Set relationships;
	private final AtomicReference fileFilterRef = new AtomicReference<>();

	private final BlockingQueue fileQueue = new LinkedBlockingQueue<>();
	private final Set inProcess = new HashSet<>(); // guarded by queueLock
	private final Set recentlyProcessed = new HashSet<>(); // guarded by
															// queueLock
	...
	private Set performListing(final File directory,final FileFilter filter, final boolean recurseSubdirectories) {
	...
	protected Map getAttributesFromFile(final Path file) {
			...
			if (store.supportsFileAttributeView("basic")) {
				try {
					...
					attributes.put(FILE_LAST_MODIFY_TIME_ATTRIBUTE,
							formatter.format(new Date(attrs.lastModifiedTime().toMillis())));
					attributes.put(FILE_CREATION_TIME_ATTRIBUTE,
							formatter.format(new Date(attrs.creationTime().toMillis())));
					attributes.put(FILE_LAST_ACCESS_TIME_ATTRIBUTE,
							formatter.format(new Date(attrs.lastAccessTime().toMillis())));
	...
	public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
		...
		if (fileQueue.size() < 100) {
			...
			if ((queueLastUpdated.get() < System.currentTimeMillis() - pollingMillis) && listingLock.tryLock()) {
				try {
					final Set listing = performListing(directory,(FileFilter)fileFilterRef.get(),
		...
		List<String> completedFileNames = new ArrayList<>();
		List<String> doneFileNames = new ArrayList<>();
		Map<String, String> doneFileExactExtensionMap = new HashMap<String, String>();
		
		for (File file1 : files) {
			String fileName = file1.getName();
			// making .done case insensitive
			if (fileName.toLowerCase().endsWith(".done")) {
				completedFileNames.add(fileName.substring(0, fileName.length() - 5));
				doneFileExactExtensionMap.put(fileName.substring(0, fileName.length() - 5),
		...
		try {
			...
			while (itr.hasNext()) {
				...
				if (completedFileNames.contains(fileName)) {
					...
					doneFileNames.add(fileName + doneFileExactExtensionMap.get(fileName));
					...
					FlowFile flowFileBackUp=session.create();
					final long importStart = System.nanoTime();
					flowFile = session.importFrom(filePath, keepingSourceFile, flowFile);
					flowFileBackUp=session.importFrom(filePath, keepingSourceFile,flowFileBackUp);
					...
					session.transfer(flowFileBackUp, REL_BACKUP);
			...
			// To take a backup on processed files doneFileNames
			final ListIterator<File> itrDones = files.listIterator();
			while (itrDones.hasNext()) {
				final File processedFile = itrDones.next();
				
				if (doneFileNames.contains(processedFile.getName())) {
					completedFiles.add(processedFile);
					final Path backupFilePath = processedFile.toPath();
					 FlowFile flowFileDone = session.create();
					final long importStart = System.nanoTime();
					flowFileDone = session.importFrom(backupFilePath, keepingSourceFile, flowFileDone);
					final long importNanos = System.nanoTime() - importStart;
					final long importMillis = TimeUnit.MILLISECONDS.convert(importNanos, TimeUnit.NANOSECONDS);

					flowFileDone = session.putAttribute(flowFileDone, CoreAttributes.FILENAME.key(), processedFile.getName());
					Map attributes = getAttributesFromFile(backupFilePath);
					if (attributes.size() > 0) {
						flowFileDone = session.putAllAttributes(flowFileDone, attributes);
					}

					session.getProvenanceReporter().receive(flowFileDone, processedFile.toURI().toString(), importMillis);
					session.transfer(flowFileDone, REL_BACKUP);
				}
			}
			session.commit();
			...
			// anything that we've not already processed needs to be put back on
			// the queue
			if (flowFile != null) {
			...
			try {
				...
				recentlyProcessed.addAll(completedFiles);
			} finally {
				queueLock.unlock();
			}
		}
